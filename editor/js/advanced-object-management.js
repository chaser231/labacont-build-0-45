...
function initAdvancedObjectManagement() {
    ...
    window.groupSelectedObjects = function() {
        ...
        // Запоминаем parentId первого объекта (или ищем макет, если не задан)
        const objects = activeObject._objects;
        let parentId = objects[0].parentId;
        
        // Если parentId не задан, проверяем, внутри какого макета находятся объекты
        if (!parentId) {
            const layouts = canvas.getObjects().filter(obj => obj.type === 'layout');
            for (const layout of layouts) {
                if (isObjectInsideLayout(objects[0], layout)) {
                    parentId = layout.id;
                    break;
                }
            }
        ...
        // Добавляем ID, имя и parentId
        group.id = `group-${Date.now()}`;
        group.name = 'Группа';
        group.type = 'group';
        group.objectType = 'group';
        group.parentId = parentId; // Устанавливаем parentId для группы
        
        // Сохраняем parentId для всех объектов внутри группы
        ...
        if (parentId) {
            ...
            if (layoutState) {
                // Удаляем старые элементы из состояния
                layoutState.elements = layoutState.elements.filter(el => 
                    !objects.some(obj => obj.id === el.id)
                );
                
                // Добавляем группу в состояние
    ...
    window.ungroupSelectedObjects = function() {
        ...
        // Запоминаем parentId группы
        const parentId = activeObject.parentId;
        const groupId = activeObject.id;
        
        // Запоминаем объекты в группе
        const objects = activeObject._objects.map(obj => {
            return {
                id: obj.id || `obj-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`,
                name: obj.name || 'Элемент группы',
        ...
        // Устанавливаем parentId для всех разгруппированных объектов
        items._objects.forEach((obj, index) => {
            // Если у объекта нет id, задаем новый
            if (!obj.id) obj.id = objects[index].id;
            if (!obj.name) obj.name = objects[index].name;
            if (!obj.objectType) obj.objectType = objects[index].type;
        ...
        if (parentId) {
            const layoutState = window.editorState.layouts.find(layout => layout.id === parentId);
            if (layoutState) {
                // Удаляем группу из состояния
                layoutState.elements = layoutState.elements.filter(el => el.id !== groupId);
                
                // Добавляем разгруппированные объекты в состояние
                objects.forEach(obj => {
                    layoutState.elements.push({
                        id: obj.id,
                        name: obj.name,
