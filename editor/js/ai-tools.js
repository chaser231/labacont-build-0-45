...
function initAITools() {
    ...
    // Модифицируем файл js/ai-tools.js

    // Функция для генерации изображения через Stable Diffusion API
    function generateImage(prompt) {
        showLoadingIndicator('Генерация изображения...');
        
        // Для демо мы будем использовать общедоступное API сервиса статичных изображений
        // В реальном приложении используйте Stable Diffusion API или другие ИИ-сервисы
        const apiUrl = `https://source.unsplash.com/featured/?${encodeURIComponent(prompt)}`;
        
        // Загружаем изображение с API
        fabric.Image.fromURL(apiUrl, function(img) {
            hideLoadingIndicator();
            
            // Если размеры не подходят, масштабируем
            if (img.width > 800 || img.height > 800) {
                const scale = Math.min(800 / img.width, 800 / img.height);
                img.scale(scale);
            }
            
            // Получаем активный макет или создаем новый
            let targetLayout;
            const activeObject = canvas.getActiveObject();
            ...
            if (activeObject && activeObject.type === 'layout') {
                ...
                targetLayout = window.createLayout(800, 600);
            }
            ...
            img.set({
                ...
                id: `image-${Date.now()}`,
                name: `ИИ: ${prompt.substring(0, 20)}`,
                objectType: 'image',
                parentId: targetLayout.id
            });
            ...
            // Добавляем в историю
            window.addToHistory();
        }, { crossOrigin: 'Anonymous' }); // Важно для загрузки с внешних доменов
    }
    ...
    function removeBackground(imageObject) {
        ...
        // В реальном приложении здесь был бы запрос к API для удаления фона
        // Для демо используем API remove.bg или аналог
        
        // Имитируем процесс с помощью Image Tracer API
        setTimeout(() => {
            // Для демонстрации используем прозрачный placeholder
            const transparentImageUrl = 'https://placehold.co/600x400/png?text=Transparent+Background';
            
            fabric.Image.fromURL(transparentImageUrl, function(img) {
                hideLoadingIndicator();
                
                // Настраиваем новое изображение с теми же параметрами, что и оригинал
                img.set({
